...
def upgrade() -> None:
        ...
        *_timestamps(),
        ...
        sa.ForeignKeyConstraint(
            ...
            [
                ...
                "model_provider_configs.organization_id",
                "model_provider_configs.project_id",
            ],
        ...
        sa.CheckConstraint(
            "total_units >= 0 AND completed_units >= 0", name="experiment_units_nonnegative"
        ),
        sa.CheckConstraint(
            "succeeded_units >= 0 AND failed_units >= 0 AND skipped_units >= 0",
            name="experiment_results_nonnegative",
        ),
    )
    ...
    op.create_index(
        "ix_experiments_project_status_created",
        "experiments",
    ...
    op.create_table(
        ...
        *_timestamps(),
        ...
        sa.ForeignKeyConstraint(
            ["project_id", "organization_id"],
            ["projects.id", "projects.organization_id"],
            name="fk_experiment_runs_project_tenant",
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ["experiment_id", "organization_id", "project_id"],
            ["experiments.id", "experiments.organization_id", "experiments.project_id"],
            name="fk_experiment_runs_experiment_tenant",
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "id", "organization_id", "project_id", name="uq_experiment_runs_tenant"
        ),
    )
    ...
    op.create_index(
        "ix_experiment_runs_project_status_created",
        "experiment_runs",
        ["organization_id", "project_id", "status", "created_at"],
    )
    ...
    op.create_table(
        ...
        *_timestamps(),
        ...
        sa.ForeignKeyConstraint(
            ...
            name="fk_experiment_run_items_run_tenant",
            ondelete="CASCADE",
        ),
        sa.ForeignKeyConstraint(
            ...
            name="fk_experiment_run_items_candidate_tenant",
            ondelete="RESTRICT",
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "id", "organization_id", "project_id", name="uq_experiment_run_items_tenant"
        ),
        sa.UniqueConstraint(
            "run_id", "candidate_item_id", name="uq_experiment_run_items_candidate"
        ),
    )
    op.create_index(
        "ix_experiment_run_items_organization_id", "experiment_run_items", ["organization_id"]
    )
    op.create_index("ix_experiment_run_items_project_id", "experiment_run_items", ["project_id"])
    op.create_index(
        "ix_experiment_run_items_run_status", "experiment_run_items", ["run_id", "status"]
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ...
            name="fk_experiment_run_attempts_item_tenant",
            ondelete="CASCADE",
        ),
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "run_item_id", "attempt_number", name="uq_experiment_run_attempt_number"
        ),
    )
    op.create_index(
        "ix_experiment_run_attempts_organization_id", "experiment_run_attempts", ["organization_id"]
    )
    op.create_index(
        "ix_experiment_run_attempts_project_id", "experiment_run_attempts", ["project_id"]
    )
    op.create_index(
        "ix_experiment_run_attempts_item_created",
        "experiment_run_attempts",
        ["run_item_id", "created_at"],
    )
...
def downgrade() -> None:
    ...
    op.drop_index(
        "ix_experiment_run_attempts_organization_id", table_name="experiment_run_attempts"
    )
