...
- Add a PostgreSQL integration test job/service in CI for migrations, composite constraints, leases, and audit immutability. Mark these tests with `@pytest.mark.integration`; default `make test` and API quality use `pytest -m "not integration"`, while `make test-integration` uses one `.ONESHELL:` Make recipe, creates a temporary Compose env file with `mktemp` from `.env.example` without touching an existing `.env`, starts `docker compose --env-file "$$COMPOSE_ENV_FILE" --profile integration up -d postgres redis`, waits with `scripts/wait-for-services.sh --compose-env-file "$$COMPOSE_ENV_FILE"`, and invokes `scripts/ci/create-test-database.sh .ci-test.env "$$COMPOSE_ENV_FILE"` (the helper uses `${GITHUB_RUN_ID:-local-<timestamp>-<pid>}` internally). The recipe sources the generated env, runs migrations and `pytest -m integration -q`, and defines a best-effort trap that saves `status=$$?`, switches to `set +e`, conditionally drops the database, stops Compose with the same `--env-file`, removes temporary env files, and exits with the saved status so cleanup cannot mask the test failure. Unit tests may use SQLite only for pure service tests; they cannot stand in for PostgreSQL constraint behavior.
...
The PostgreSQL integration job creates a temporary Compose env file from `.env.example` with `mktemp` (never overwriting or deleting a user `.env`) and starts Compose with `docker compose --env-file "$COMPOSE_ENV_FILE" --profile integration up -d postgres redis`. It then calls `scripts/wait-for-services.sh --compose-env-file "$COMPOSE_ENV_FILE"` before `scripts/ci/create-test-database.sh .ci-test.env "$COMPOSE_ENV_FILE"`. The helper uses `docker compose --env-file "$COMPOSE_ENV_FILE" exec -T postgres psql` with the Compose credentials, chooses `GITHUB_RUN_ID` in CI or a generated `local-<timestamp>-<pid>` fallback, and writes `DATABASE_URL`, `TEST_DATABASE_NAME`, and `TEST_DATABASE_HOST` to `.ci-test.env`; it does not rely on child-shell exports. Creation, Compose startup, readiness, lock check, migration, test, and cleanup all run in one CI `run: |` block so `COMPOSE_ENV_FILE` is never expected to survive a step boundary. That block sources the temporary env files, exports `APP_ENV=development DEV_ACTOR_ID=<fixed test UUID>`, runs `uv lock --directory apps/api --check`, `uv sync --directory apps/api --locked`, migrations, and `pytest -m integration -q`; its trap is an idempotent `set +e` cleanup function that checks for `.ci-test.env` before calling `scripts/ci/drop-test-database.sh .ci-test.env "$COMPOSE_ENV_FILE"`, then always stops the Compose profile with the same `--env-file` and removes only temporary env files without masking the original exit code. `scripts/wait-for-services.sh` is extended to require/pass through the same `--compose-env-file`, and `drop-test-database.sh` exits 0 when the env file or target database does not exist.
